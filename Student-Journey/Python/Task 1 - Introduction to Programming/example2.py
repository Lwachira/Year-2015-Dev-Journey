...
# Now that you've learnt how to run a Python file, you should learn about errors.
...
# Be sure to close all quotes (" ... ") and all brackets ( [ ] )! These are the most common errors.
...
# The most common way to output things is to print them. 
...
# 	In the first line, type: name = raw_input("Enter your name"). Note that later versions of Python use 'input' and not 'raw_input' - make sure you have the right version!
...
# 	The program will then output your name which was automatically stored in the variable name which you specified it should print out.
...
# 	Run the file in IDLE. Note the errors, fix them and get the program to run and finally print out the message.
...
# 	HINT: Note that IDLE will only point out these errors one at a time (in the order it encounters them).
