...
#  *** PLEASE STOP READING THIS NOW, OPEN THE INSTALLERS FOLDER IN THIS DIRECTORY AND RUN BOTH FILES TO INSTALL NOTEPAD++ AND PYTHON. ***
...
#  This is the start of a course in programming. If you successfully complete the few basic tasks you'll be learning about exciting topics such as Artificial Intelligence.
#  Python is one of the easiest languages to learn how to program in. Programming takes patience and practice, and we hope to guide you through this with our course.
...
# Python is VERY similar to Java and most programming languages have the same structure so if you learn one - you can learn more easily! It's not like learning human languages.
# Almost all programming languages share common SYNTAX - the way the statements are written.

# ========= Variables ===========
#  A variable is a way to store information. 
#  Variables are created in a part of your computer memory and this area is reserved for the use of the variable in your program while the program runs.
...
#  A: Not really. There are restrictions on variables in terms of naming so you have to comply to these rules. Letters and underscores are permitted and anything else should generally be avoided.
...
# 	If you close this black box, and open a new one and type: print name , you will get an error. This is because you were coding Python 'Shell' and your variables aren't saved. 
# 	We write Python in text editors like Notepad++ or the IDLE Python GUI so that all our variable definitions and code are saved. We can then run these files as python programs at any time we want, and we can use these 
...
#      Integers store whole numbers and are also known as ints. Floats store decimal numbers and are sometimes called doubles. Strings store text (which is a fancy character array).
...
#      Take heed that types can be converted from one to another, but you need to take care when setting a string with numerical information for example, and your intention thereof.
...
#  If you add string objects they will be CONCATENATED (put side by side or joined). You cannot subtract or divide String variables - that's only for ints or Floats. 
...
# Now your code will open in the Python GUI IDLE. Press F5 on your keyboard to RUN the Python and the output will appear in a separate python output (Python Shell) window!
...
# Play around with this program in IDLE. Change statements and delete things, then run the code and see what happens.
