...
# We check if the age is greater than or equal to 18, if the age meets the condition, then the person is allowed inside and if they are not 18 and older, then they are too young.
...
# A: We can have multiple conditions, it is quicker than writing an if statement within an if statement or using and else if statement to make your program work in a certain way.
...
# If  myName is Tom and myFavouriteColour is orange, then the if statement's print statement will execute. Both conditions need to be met otherwise the else statement and its subordinate will execute.
...
# We also have the concept of branching where the first condition that is true will be executed irrespective of whether or not the the subsequent conditions are true and the subsequent conditions are skipped anyway.
...
if item == "dog" or len(item) == 5:		# Branch 1
	...
	print "Your item is both a chair and len of item i 5"
...
# In this for loop below, the condition is that when the variable i (which is an integer) is in the range of 1 to 10 (i.e. either 1, 2, 3 ,4 ,5 ,6 ... or 10) , the code under
